"""
Summarizer Service — TextRank extractive summarization using NetworkX.
Builds sentence similarity graph, runs PageRank, returns top N sentences.
No LLMs. Pure graph-based NLP.
"""
import re
import logging
import numpy as np
...
def _clean_text(text: str) -> str:
    """Clean up missing spaces and camelCase run-ons from scraping/PDFs."""
    # Add space after periods if missing: "word.Another" -> "word. Another"
    text = re.sub(r'([.!?])([A-Za-z])', r'\1 \2', text)
    # Split some bad camel cases "forDataStructures" -> "for Data Structures"
...
def textrank_summarize(text: str, n: int = 8) -> list[str]:
    ...
    3. Compute pairwise cosine similarity → similarity matrix
    4. Build similarity graph using NetworkX
    5. Run PageRank to score sentences
    6. Return top N sentences (in original order)

    Returns list of bullet-point sentences.
    """
    ...
    try:
        ...
        # Accumulate sentences until we hit ~550 words (or run out)
        target_words = 550
        current_words = 0
        selected_indices = []
        ...
        for idx in ranked_indices:
            ...
            if current_words + sent_words > target_words + 50: # Provide a small buffer overhead
                break
